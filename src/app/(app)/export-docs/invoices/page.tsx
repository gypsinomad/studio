...
import { PlusCircle, Search, Filter, FileText, Download, Eye } from 'lucide-react';
...
export default function ExportInvoicesPage() {
  ...
  // Fetch real documents from Firestore filtered by invoice-related types
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Invoices & Docs" description="Generate and track export invoices, packing lists, and certificates.">
        <Button asChild className="rounded-xl"><Link href="/export-docs/invoices/new"><PlusCircle className="mr-2 size-4" /> Create Invoice</Link></Button>
      </PageHeader>
      ...
      <div className="flex items-center justify-between gap-4 bg-white/50 p-4 rounded-2xl border border-stone-200/60 shadow-sm backdrop-blur-sm">
        <div className="flex gap-2 flex-1">
          <div className="relative w-64">
            ...
            <Input placeholder="Search records..." className="pl-10 bg-white/80 h-10 border-stone-200 rounded-xl" />
          </div>
            ...
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={5} className="text-center py-16 text-stone-400 font-medium">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p>No invoices or shipping documents found.</p>
                    <p className="text-xs font-normal">Upload documents in an Export Order to see them here.</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : (
              documents.map((doc) => (
                <TableRow key={doc.id} className="group hover:bg-stone-50/50 transition-colors">
                  <TableCell className="pl-6 font-bold text-primary hover:underline cursor-pointer" onClick={() => handleViewPdf(doc.fileUrl)}>
                  ...
                  <TableCell>
                    <Badge variant="outline" className={doc.status === 'finalized' ? 'bg-emerald-50 text-emerald-700 border-emerald-100 px-2 rounded-lg' : 'bg-sky-50 text-sky-700 border-sky-100 px-2 rounded-lg'}>
                      {doc.status}
                    </Badge>
                  </TableCell>
